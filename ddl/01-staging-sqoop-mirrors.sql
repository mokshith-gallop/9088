-- ----------------------------------------------------------------------------
-- 01-staging-sqoop-mirrors.sql
-- BigQuery DDL for staging: Sqoop-landed RDBMS mirrors (27 tables).
-- Translated from hive/ddl/02-staging-sqoop-mirrors.hql.
--
-- Type mapping: BIGINT/INT→INT64, STRING→STRING, BOOLEAN→BOOL,
--               DOUBLE→FLOAT64, DECIMAL(p,s)→NUMERIC(p,s).
-- Hive PARTITIONED BY (load_date STRING) → inline load_date DATE
--   + PARTITION BY load_date.
-- Hive EXTERNAL, STORED AS, LOCATION, TBLPROPERTIES → stripped.
-- Hive COMMENT → OPTIONS(description='...').
-- Hive CLUSTERED BY (col) INTO N BUCKETS → CLUSTER BY col (no count).
-- ----------------------------------------------------------------------------
...
-- stg_wfm_schedule: Hive multi-column partition (load_date, site_code)
-- → BQ DATE partition on load_date + CLUSTER BY site_code.
-- Both former Hive partition columns are promoted to inline columns.
...
-- stg_tel_call: Hive CLUSTERED BY (call_id) INTO 16 BUCKETS → CLUSTER BY call_id.
...
-- stg_fin_invoice: contains the LIE columns (issued_ts_sec, due_ts_sec)
-- whose names say "seconds" but values are actually MILLISECONDS.
